...
export function CaseHeader() {
  const { actionNeeded, caseView } = useCasePage();
  const referenceNumber = caseView.visaCase.referenceNumber ?? caseView.caseSubmission?.referenceNumber;

  return (
    <header className="flex items-center gap-4 border-b border-line bg-panel px-5 py-3">
      <div>
        <SplitFlap status={caseView.visaCase.internalStatus} />
      ...
      {referenceNumber ? (
        <div className="font-mono text-[10px] tracking-[0.14em] text-ink2">
          REF <b className="text-ink">{referenceNumber}</b>
        </div>
      ) : null}
    </header>
  );
