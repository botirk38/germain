...
## Consolidated Workflow (7 tools, strict order)

1. **evaluateCase** → Assess eligibility, select visa route, generate document checklist. One tool replaces three steps.
2. **uploadDocuments** → UI tool: collect required documents from user.
3. **reviewAndPrepare** → Review documents, generate application form, create cover letter, itinerary, and proof of ties. One tool replaces three steps.
4. **runRiskReview** → Final risk assessment, calculate approval likelihood, flag remaining issues.
5. **submitApplication** → UI tool: Opens Browser Use session to fill the consulate portal. User watches live. Handles appointment booking and fee payment.
6. **approveSubmission** → UI tool: HUMAN APPROVAL GATE. User reviews filled form and explicitly approves or rejects.
7. **monitorCase** → Check user's Gmail for embassy correspondence (RFEs, biometrics scheduling, decisions).
